...
# Design System: Aayush Tiwari — Portfolio
...
Figures are ink line work: no fills beyond the two stock steps, stroke widths 0.7 (grid) to 1.15 (connectors) with `vector-effect: non-scaling-stroke`, and a standing CSS guard forcing `rx: 0; ry: 0` on every `rect` in every canvas — the attributes are gone from the SVG source too, and the guard exists so a re-added one cannot ship.

### Named Rules

**The No-Cards Rule.** Nothing in page structure is enclosed in a bordered or filled box. This governs content containers and page composition. Ruled cells inside a diagram are exempt: they are figure notation, and that exemption was raised, argued and accepted in review rather than assumed.
...
- **Shape:** Square (0 radius); a 1px `rule-strong` bottom border with 1px of padding beneath the text.
- **Default:** Full Ink, weight 500, inline display so the rule breaks with the line.
- **Hover:** Border colour promotes to full Ink over 160ms. Non-`.link` anchors instead get a 1px underline at `0.22em` offset.
- **Mono link variants** (work links, reference list, footer, secondary CTA): mono at 0.6875–0.75rem with the same rule-and-promote behaviour; footer links start at the weaker `rule` hairline and gain both colour and border on hover.

### Chrome Buttons (command trigger, theme toggle, menu trigger)
- **Character:** Instrument panel, not UI furniture.
- **Shape:** Square, 30px tall, 1px `rule` border, no fill at any state.
- **Default:** Ink Secondary label in mono at 0.6875rem; the toggle is a 30×30 square with sun and moon stacked in one grid cell.
- **Hover:** Border to `rule-strong`, text to Ink. No fill, no lift, no shadow.
- **Theme toggle state:** The inactive icon fades to 0 opacity and `scale(0.7)` over 180–220ms; the pair never translates.

### Metadata Block (the recruiter's answer)
- **Character:** A tabular field list, the top of a real document.
- **Structure:** `6.5rem` mono uppercase term column against a 0.875rem definition column, each row bounded by a hairline, 11px of vertical padding.
- **Status row:** The one amber field in the block. It fills the full row edge to edge (`padding-inline: 12px; margin-inline: -12px`), sets its value at weight 600 in Mark Ink, mutes its own term to a 68% Mark Ink mix, and drops its bottom hairline. This is the canonical demonstration of Colour-Is-The-Sheet: a filled row, not a highlighter stroke across the words.
...
### Figures
- **Character:** Plates in a document. Every one states what it measures and what it does not.
- **Frame:** No frame. `clamp(6px, 1vw, 12px)` of vertical breathing, full-width SVG or image, nothing around it.
- **Caption:** 46ch max, 18px below the plate, indented 26px behind a 16px `rule-strong` leader line drawn at `0.62em`. A mono uppercase `Figure n` label sits on its own line above the caption text.
- **Token scoping trap:** `components/visuals/visuals.module.css` declares its own `--v-*` tokens (`--v-text`, `--v-line`, `--v-muted`, `--v-cyan`, `--v-acid`, `--v-warn`) **on `.visual` itself**, not on an ancestor. A theme or page-level override of those names will not reach a figure, because the element's own declaration wins over any inherited value. To retarget figures, override on `.visual`/`.project-visual-root` or change the `--v-*` defaults in the module. `.project-visual-root` in globals.css re-declares the same set for exactly this reason.

### Evidence Row
- **Structure:** A `13rem` mono measurement column against a label column; the qualification sits beneath the label at 0.8125rem behind its own 14px leader line. The number is large and tabular; its boundary always travels with it.
...
- **Input:** Borderless, transparent, body face at 0.9375rem, no focus ring on the field itself — the dialog is the focus context.
- **Item states:** Selected rows take `Stock Tint`, full Ink, weight 600. Group headings and shortcut hints are mono micro-labels at Ink Muted.
...
### Focus & Selection
- 2px solid Ink outline at 3px offset globally; in dark theme the ring switches to amber; on the amber field it switches to Mark Ink. Text selection paints the amber sheet with Mark Ink type. The skip link is a hard-edged Ink block that slides in on `:focus` (not `:focus-visible`, so programmatic focus also reveals it).

### Print
The header, footer, skip link and close field are removed and the body reverts to black on white — the document survives as a document.
